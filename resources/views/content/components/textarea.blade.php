<div class="col-12 mb-3">
    ...
    @endphp
    <textarea name="{{ $attr->field_name }}" id="{{ $attr->field_name }}" class="@error('name') is-invalid @enderror form-control" rows="3" {{ $attr->is_required ? 'required' : ''}} {{ isOwnerStep($attr->step_id)==false || $current_step != $attr->step_id || $status != 'progress' ? ' disabled' : '' }}>{{ old($attr->field_name) ?? ($value ? $value : $d) }} </textarea>
