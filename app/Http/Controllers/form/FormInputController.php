...
class FormInputController extends Controller
{
    ...
    {
        $results = FormInput::select('events.*','form_inputs.uniq_id','form_inputs.current_step','form_inputs.status','form_inputs.form_input_id_before','users.name')
        ->where([
    ...
    public function store(Request $request)
    {
        ...
        FormInput::create([
            ...
            'submit_id' => "FSKP" . date('Ymd') . str_pad($cek->count + 1, 2, "0", STR_PAD_LEFT). str_pad(auth()->user()->id, 4, "0", STR_PAD_LEFT),
    ...
    {
        $data = StepField::where('step_id', $step_id)->get();

        $headform = FormInput::select('form_inputs.*','events.event','events.description','event_steps.event_step')->where('uniq_id', $uniqid)->where('event_steps.id', $step_id)
        ->join('events', 'form_inputs.event_id', '=', 'events.id')
        ->join('event_steps', 'event_steps.event_id', '=', 'events.id')->first();
        $steps = EventStep::select('event_steps.*')->where('event_id', $headform->event_id)->get();
        ...
        //dd($data_verif);
        $data_value=[];
        $data_value = FormInputDetail::where('form_input_id', $headform->id)->where('step_id', $step_id)->orderby('field_order','asc')->get();
        return view('content.forminput.form', compact('data','data_value','headform','steps','result','render','step_id','data_verif'));
    }
    ...
    public function processNextStep(Request $request, string $uid)
    {
        ...
        $head = FormInput::select('form_inputs.*', 'events.event', 'users.name')
            ->join('events', 'form_inputs.event_id', '=', 'events.id')
            ->join('users', 'form_inputs.user_id', '=', 'users.id')
            ->where('form_inputs.uniq_id', $uid)
            ->first();

        $eventStep = EventStep::find($head->current_step);

        $previousStepName = EventStep::find($request->prev_step)->event_step;

        if ($eventStep->owner != 'admin' && $eventStep->step_owner == auth()->user()->roles()->first()->name) {
            sentNotifStepChange($head->user_id, $head->name, $head->submit_id, $previousStepName, $eventStep->event_step, $head->event);
        }

        return redirect()->route('forminput-submit', [$head->uniq_id, $request->current_step]);
    }

    public function processBackStep(Request $request, string $uid)
    {
        $request->validate([
            'current_step' => 'required',
            'prev_step' => 'required',
    ...
    public function processFinishStep(Request $request, string $uid)
    {
        ...
        $head = FormInput::where('uniq_id', $uid)
            ->select('form_inputs.*', 'events.event', 'users.name')
            ->join('events', 'form_inputs.event_id', '=', 'events.id')
            ->join('users', 'form_inputs.user_id', '=', 'users.id')
            ->firstOrFail();

        $eventStep = EventStep::where('id', $head->current_step)->firstOrFail();
        ...
        if ($eventStep->owner != 'admin' && $eventStep->step_owner == auth()->user()->roles()->first()->name) {
            if ($head->status == 'success') {
                sentNotifSuccess($head->user_id, $head->name, $head->submit_id, $eventStep->event_step, $head->event);
            } else {
                sentNotifFail($head->user_id, $head->name, $head->submit_id, $eventStep->event_step, $head->event);
            }
        }
    ...
    public function export(string $uniqid)
    {
        ...
        $user = FormInput::
                join('users', 'users.id' , 'form_inputs.user_id')
                ->join('user_profiles', 'users.id', '=', 'user_profiles.user_id')
                ->where('uniq_id', $uniqid)->first();

        $templateProcessor = new \PhpOffice\PhpWord\TemplateProcessor(app_path('Exports/Templates/suket_penelitian.docx'));

        $templateProcessor->setValue('date', date("d-m-Y"));
        //$templateProcessor->setValue('petugas', $results->officer_name);
        ...
        $templateProcessor->setValue(
            [
                ...
                'team_leader', 
                'team_members', 
                'number_applicants_research_letter', 
                'date_applicants_research_letter', 
                'regard_applicants_research_letter', 
                'start_at', 
                'finish_at'
            ],
            [
                $result->applicantsjob_id, 
                $result->institution, 
                ...
                $result->team_leader, 
                $result->team_members, 
                $result->number_applicants_research_letter, 
                $result->date_applicants_research_letter, 
                $result->regard_applicants_research_letter, 
                $result->start_at, 
                $result->finish_at
            ]
        );
