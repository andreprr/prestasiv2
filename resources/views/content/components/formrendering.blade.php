...
<div class="card w-100 position-relative overflow-hidden">
    ...
    <div class="card-body p-4 bg-light">
        @if($headform->current_step == $step_id && $headform->status == 'progress')
            ...
            @foreach($data as $key => $row)
                <div class="card-body p-4 border border-1 rounded-3 mb-2 bg-white">
                        ...
                        @case('file')
                            ...
                            @break
                        @case('date')
                            @include('content.components.dateinput', ['attr' => $row, 'value' => getValue($headform->id, $row->step_id, $row->field_name), 'current_step' => $headform->current_step, 'status' => $progress->event->status])
                            @break
                        @default
                ...
                <div class="card-body p-4 border border-1 rounded-3 mb-2 bg-white">
                    <div class="row">
                        <div class="col-4">
                            ...
                            @case('date')
                            <p class='mt-2 mb-3'>
                                {{ \Carbon\Carbon::parse($row->value)->locale('id')->translatedFormat('l, d F Y') }}
                            </p>
                            @break
                            @case('select')
                            <p class='mt-2 mb-3'>{{ $row->value }}</p>
                            @break
                            @case('file')
            ...
            <div class="col-12 d-flex mt-3 justify-content-between ">
                ...
                @if($progress->event->current_step == $step_id && isOwnerStep($progress->event->current_step)==true && $headform->status == 'progress')
                    ...
                    @if(nextStep($headform->event_id, $step_id))
                        <div>
                            <form id="formProcess" action="{{ route('forminput-process-step',$headform->uniq_id) }}" method="POST">
                                ...
                                <input type="hidden" name="prev_step" value="{{$progress->event->current_step}}">
                            </form>
                    ...
                    @if(Auth::User()->roles()->first()->name == 'admin')
                        <div>
                            <form id="formBackProcess" action="{{ route('forminput-backprocess-step',$headform->uniq_id) }}" method="POST">
                                @csrf
                                <input type="hidden" name="current_step" value="{{prevStep($headform->event_id, $step_id)}}">
                                <input type="hidden" name="prev_step" value="{{$progress->event->current_step}}">
                            </form>
                            <a id="prosess_back_button" onclick="prosesback_button()" class="btn btn-md btn-danger ms-2 d-inline-block" data-toggle="tooltip" data-placement="top" title="Next">Kembalikan Ke Tahap Sebelumnya <i class="ti ti-circle-chevron-right"></i></a>
                        </div>
                        <div>
...
<script>
    ...
    function prosesback_button(){
        if (confirm('Apakah anda yakin semua data sudah benar dan akan memproses ke tahap selanjutnya?')){

            $('#formBackProcess').submit();
        }
    }
